/****** Object:  Procedure [dbo].[S_TEST1]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[S_TEST1] 
	@input varchar(max)='',
	@spname varchar(50)='',
	@section varchar(50)='',
	@usrcomid varchar(50)='',
...
DECLARE 
	...
	@dt1 varchar(max)='',
	@dt2 varchar(max)=''
if(isnull(@input,'')='') set @input='{}'
select [key]=a.[key], [value]=a.[value] into #tjsonroot from openjson(@input) a
select  @optype=optype, @trnid=trnid, @spname=spname, @section=section,
...
(
 min([value])
 for [key] in (optype, trnid, spname, section,
...
B:
BEGIN
select cast(compress('a') as varbinary(max)) 
return
	SELECT a.c1 from #dt1 a
	SELECT * from #dt2
return
END
--==================================End Section==================================
